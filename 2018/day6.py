...
def part1(lines):
    ...
    coord_id_to_point = {coord_id: point for coord_id,
                         point in enumerate(coords, start=1)}
    region_sizes = defaultdict(int)
    infinite_ids = set()

    for i in range(max_r + 1):
        for j in range(max_c + 1):
            min_dists = sorted([(abs(r - i) + abs(c - j), coord_id)
...
def part2(lines, manhattan_limit=10000):
    ...
    for i in range(max_r + 1):
        for j in range(max_c + 1):
            size_shared_region += int(sum(abs(r - i) + abs(c - j)
                                          for r, c in coords) < manhattan_limit)
